...
Let's start with this PSA: **loops aren't bad!**
...
This is obviously a trivial example, but you get the idea: these are three ways of going through a thing, and applying a function to each value of that thing. 


The (main) advantages of using things like `apply` or `map` though are though that they
...
How do we figure out which tech's are the problem? We can use `purrr`'s `transpose` to make things a little easier to look at

```{r}

diagnose_length <- diagnose_length %>% 
 purrr::transpose() 

listviewer::jsonedit(diagnose_length)

bad_techs <-  diagnose_length$error %>% 
  keep(~is.null(.) == F)

names(bad_techs)
```

I leave it to your imagination to think of how to resolve this problem, but at least we now know where the problem is.
...
Factors can creep into your data, especially if you're reading in .csv's using base R, or getting data from other people.  There's lot's of ways to solve this, but you can use `purrr` to efficiently check for factors, and convert them to characters in your data frame.
